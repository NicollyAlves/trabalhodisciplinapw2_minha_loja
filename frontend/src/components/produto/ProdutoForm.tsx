...
function ProdutoForm({ handleSubmit, produto }: Props){
    ...
    return (
        <form onSubmit={onSubmit} >
            ...
            <Box sx={{mb:2}} >
                <TextField label="Preço" required type="number" value={preco ?? ""} onChange={(e) => setPreco(parseFloat(e.target.value))}  />
            </Box>
            <Box sx={{mb:2}} >
                <TextField label="Estoque" required type="number" value={estoque ?? ""} onChange={(e) => setEstoque(parseInt(e.target.value))}  />
            </Box>
